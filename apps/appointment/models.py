...
class AppointmentsManager(models.Manager):
    def create_appointment(self, request):
        ...
        if request.POST["task"] == "":
            ...
            newappointment = self.create(date=request.POST['dates'], task=request.POST['task'],time=request.POST['time'],schedule=user)
            error = False
        return error
...
class Appointments(models.Model):
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    schedule = models.ForeignKey('User', related_name='schedule')
    appointmentsManager = AppointmentsManager()

class Status(models.Model):
    status = models.CharField(max_length=100)
    add_status = models.ForeignKey('Appointments',related_name='add_status')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
